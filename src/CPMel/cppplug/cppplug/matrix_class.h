...
#define NewMatrixObject CPStaticNewPyObject(Matrix_Type, Matrix)
...
//普通方法
static PyObject* Matrix_compile(Matrix* self);
...
*/
static PyMethodDef Matrix_SpamMethods[] = {
    {"compile",
    (PyCFunction)Matrix_compile,
    METH_NOARGS,
    "返回编译完成的值"
    },
    {0, 0, 0, 0}
...
static PyObject* Matrix_New(PyTypeObject* type, PyObject* args, PyObject* kw)
{
    ...
    if (len == 4) {
        ...
        double* val_ptr = (double*)data->val;
        for (unsigned int i = 0; i < 16; i++) {
            val_ptr[i] = PyFloat_AsDouble(PyObject_GetItem(args, PyInt_FromLong(i)));
        }
    }
    else {
        PyErr_SetString(PyExc_ValueError, "仅支持（（int*4）*4）或 （int * 16）");
...
static PyObject* Matrix_GetItem(Matrix* self, PyObject* item)
{
    if (PyObject_Length(item) == 2)
    {
        ...
        double * i = (double *)self->val;
        return PyFloat_FromDouble(i[_PyInt_AsInt(item)]);
    }
    else {
...
static PyObject* Matrix_compile(Matrix* self) {
    ...
    for (unsigned int i = 0; i < 16; i++) {
        PyTuple_SetItem(tuple, i, PyFloat_FromDouble(((double*)self->val)[i]));
    }
    return tuple;
}
